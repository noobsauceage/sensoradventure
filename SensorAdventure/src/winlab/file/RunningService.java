...
public class RunningService extends Service implements SensorEventListener {
	...
	public void onCreate() {
		...
		for (int i = 0; i < 13; i++)
			...
			if (SensorSetting.sensors[i]) {
				...
				if (AndroidSensors.dataConfig[0]) {
				...
				if (AndroidSensors.dataConfig[2]) {
					...
					try {
						...
						captureFiles.add(new PrintWriter(new FileWriter(myFile,
								false)));
						captureFiles.get(count).println(
								"`timestamp`" + ","
										+ getFields(mSensor.getType()));
	...
	public void onSensorChanged(SensorEvent event) {
		...
		if (AndroidSensors.dataConfig[0])
			...
			try {
				...
				// Switch on sensor. Each of these sensors print out x,y,z
				// values.
				switch (i) {
				case 1:
				case 2:
	...
	public void onDestroy() {
		for (int i = 0; i < 13; i++)
			...
			// must delete them.
			else if (AndroidSensors.dataConfig[0]) {
				if (file[i].exists())
					file[i].delete();
			}
