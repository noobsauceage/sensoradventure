...
public class SensorMonitor extends Activity implements SensorEventListener {
	...
	protected void onStart() {
		...
		for (int i = 0; i < sensors.size(); ++i) {
			...
			try {
				...
				dataDir.mkdirs();
				// Make the file
				File myFile = new File(dataDir, ourSensor.getType() + "_"
						+ ourSensor.getName() + ".csv");
				...
				// csv field names
				// captureFiles.get(i).println("`timestamp`" + "," +
				// "`values[0]`" + "," + "`values[1]`"+ "," + "`values[2]`");
				captureFiles.get(i).println(
						"`timestamp`" + "," + getFields(ourSensor.getType()));
	 ...
	 * startId) { sensorManager = (SensorManager)
	 * getSystemService(SENSOR_SERVICE); //List<Sensor> sensors =
	 * sensorManager.getSensorList( Sensor.TYPE_ALL ); sensor =
	...
	/*
	 * // Might need this for SQLite private class SensorEventLoggerTask extends
	 * AsyncTask<SensorEvent, Void, Void> {
	 * 
	 * @Override protected Void doInBackground(SensorEvent... events) {
	 * SensorEvent event = events[0]; // log the
	 * value****************************************************** ContentValues
	 * values = new ContentValues(); values.put(SensorDataHelper.COL_VALUE,
	 * value); values.put(SensorDataHelper.COL_TIMESTAMP, timestamp);
